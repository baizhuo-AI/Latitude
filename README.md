...
- **多窗口**:对话悬浮条(全局快捷键唤起)、待办悬浮窗、启动器;与主窗共享 SQLite,跨窗口同步走 Tauri 事件(早期的 BroadcastChannel 方案在 Tauri 多窗口下不生效,已弃用)
...
**本项目将以开源形式发布**（总纲 §1.2、§6.2）。具体协议待定，将在首次公开发布前确定并补齐 LICENSE 文件。

在此之前：仓库虽为 public，但尚未授予任何开源许可，保留所有权利。
