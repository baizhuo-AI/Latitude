...
- Storage and control are local-first. Selected model-provider inference and DeepSeek web search are explicit external outbound operations.
- The Agent Host defaults to `deepseek-official` / `deepseek-v4-flash`, with `DEEPSEEK_MODEL` as the deployment default override. Browser Settings can select a configured DeepSeek, OpenAI, or Anthropic provider/model for the next turn; the credential-free selection is stored in Agent-owned `provider-settings.json`, while keys remain write-in-environment only. The supported `dev:local` launcher filters credential-shaped variables from the Browser/Domain parent environment and re-adds only the explicit DeepSeek/OpenAI/Anthropic provider allowlist to the Agent Host; the Domain launcher filters again before entering Rust. Service scripts and Vite still read `.env.local`, while only `VITE_*` reaches the Browser bundle, so `.env.local` is not a general-purpose secrets file and no model credential may use a `VITE_*` name.
...
- Candidate cadence: a typed intervention moves only by an explicit `proposed → touched → shaping → concluded / parked` command. A user can reopen `parked → touched` with `touch`; reopening restores collaboration without confirming a conclusion. Three days of proposed silence and seven days in shaping each create at most one durable prompt receipt; acknowledging delivery does not advance or park the candidate.
